...
class StoriesSingle extends Component {
  constructor(props) {
      super(props);
      this.state = {
        data: '',
  ...
  shareTwitter(){
    window.open('https://twitter.com/share?text='+ this.state.data[0].title +'?&amp;url='+encodeURIComponent(window.location.href)+'&amp;via=solaritycu','twitterShare','width=626,height=436');
    mixpanel.init('d96d9f1409ced72777048f912ef7591e');
    mixpanel.track('Shared Blog Post', {
  ...
  mailToLink(){
    window.open("mailto:?subject=" + this.state.data[0].title + "&body=Check out this site " + window.location.href);
    mixpanel.init('d96d9f1409ced72777048f912ef7591e');
    mixpanel.track('Shared Blog Post', {
  ...
  const RELATED_QUERY = gql`
    ...
    const BASIC_PAGE_QUERY = gql`
      {
         entry(slug:"${this.props.slug}", section:[stories]){
          title
          ...on Stories{
            ${metaQuery.metaQuery}
            ${heroQuery.heroQuery}
    ...
    return (
        <React.Fragment>
          ...
          <div className="align-center block-center mb-8">
            <a href="/stories" className=" button button-primary fontSize0 "> Read More Stories </a>
          </div>
...
{/*
>>>>>>> 6ce3c09602124acbef8617bc5090104c499f191d
